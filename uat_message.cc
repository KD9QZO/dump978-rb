...
void AdsbMessage::DecodeMS(const RawMessage &raw) {
	...
	if (!raw_callsign.empty()) {
		if (raw.Bit(27, 7)) { // CSID field, 1 = callsign, 0 = flightplan ID (aka squawk)
			...
			if (raw_callsign.size() == 4 && IsOctal(raw_callsign[0]) && IsOctal(raw_callsign[1]) && IsOctal(raw_callsign[2]) && IsOctal(raw_callsign[3])) {
...
#define EMIT(x)																\
	...
	if (aircraft_size)
		o["aircraft_size"] = {
			{ "length", aircraft_size->first },
			{ "width", aircraft_size->second }
		};
	...
	if (emitter_category) {
		o["emitter_category"] = std::string{
			(char)('A' + (*emitter_category >> 3)),
			(char)('0' + (*emitter_category & 7))
		};
	}
