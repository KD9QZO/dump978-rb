...
namespace airnav::uat {
	...
	struct AircraftState {
		AircraftState(AddressQualifier aq = AddressQualifier::INVALID, AdsbAddress ad = 0): address_qualifier(aq),
			address(ad) {
			rssi.fill(0.0);
		}
	...
	public:
		...
		static Pointer Create(boost::asio::io_service &service, std::chrono::milliseconds timeout = std::chrono::seconds(300)) {
		...
		const MapType &Aircraft() const {
			return aircraft_;
		}

		std::uint32_t TotalMessages() const {
	...
	private:
		Tracker(boost::asio::io_service &service, std::chrono::milliseconds timeout): service_(service),
			strand_(service),
			timer_(service),
			timeout_(timeout) {
		}
