...
std::tuple<bool, Bytes, unsigned> FEC::CorrectUplink(const Bytes &raw, const std::vector<std::size_t> &erasures) {
	...
	// uplink messages consist of 6 blocks, interleaved; each block consists of a data section then an ECC section; we need to deinterleave, check/correct the
	// data, then join the blocks removing the ECC sections.
