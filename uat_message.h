...
namespace airnav::uat {
	...
	public:
		...
		RawMessage(): type_(MessageType::INVALID),
			received_at_(0),
			errors_(0),
			rssi_(0),
			raw_timestamp_(0) {
		...
		RawMessage(const Bytes &payload, std::uint64_t received_at, unsigned errors, float rssi, std::uint64_t raw_timestamp = 0): payload_(payload),
			received_at_(received_at),
			errors_(errors),
			rssi_(rssi),
			raw_timestamp_(raw_timestamp) {
		...
		RawMessage(Bytes &&payload, std::uint64_t received_at, unsigned errors, float rssi, std::uint64_t raw_timestamp = 0): payload_(std::move(payload)),
			received_at_(received_at),
			errors_(errors),
			rssi_(rssi),
			raw_timestamp_(raw_timestamp) {
		...
		RawMessage(MetadataMap &&metadata): type_(MessageType::METADATA),
			received_at_(0),
			errors_(0),
			rssi_(0.0),
			raw_timestamp_(0),
		...
		RawMessage(const MetadataMap &metadata): type_(MessageType::METADATA),
			received_at_(0),
			errors_(0),
			rssi_(0.0),
			raw_timestamp_(0),
		...
		std::uint64_t RawTimestamp() const {
	...
	struct CapabilityCodes {
		...
		bool operator==(const CapabilityCodes &o) const {
			return (uat_in == o.uat_in && es_in == o.es_in && tcas_operational == o.tcas_operational);
		}
	...
	struct OperationalModes {
		...
		bool operator==(const OperationalModes &o) const {
			return (tcas_ra_active == o.tcas_ra_active && ident_active == o.ident_active && atc_services == o.atc_services);
		}
	...
	struct ModeIndicators {
		...
		bool operator==(const ModeIndicators &o) const {
			return (autopilot == o.autopilot && vnav == o.vnav && altitude_hold == o.altitude_hold && approach == o.approach && lnav == o.lnav);
		}
