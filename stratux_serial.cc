...
StratuxSerial::StratuxSerial(boost::asio::io_service &io_service, const std::string &path): io_service_(io_service),
	path_(path),
	port_(io_service),
...
void StratuxSerial::ParseInput(const Bytes &buf) {
	...
	static const std::array<std::uint8_t, 4> preamble = {
		0x0A,
		0xB0,
		0xCD,
		0xE0
	...
	for (auto i = buf.begin(); i != buf.end();) {
		switch (parser_state_) {
			...
			default: assert("impossible state" && false);
		}
	}
...
void StratuxSerial::HandleError(const boost::system::error_code &ec) {
	DispatchError(ec);
}
