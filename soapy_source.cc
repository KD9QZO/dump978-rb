...
static SoapySDR::Kwargs KwargsFromString(const std::string &markup) {
	return SoapySDR::KwargsFromString(markup);
}
#else
// Compatibility shim
...
SoapySampleSource::SoapySampleSource(boost::asio::io_service &service, const std::string &device_name, const boost::program_options::variables_map &options): timer_(service),
...
void SoapySampleSource::Init() {
	try {
		...
		device_ = {
			SoapySDR::Device::make(device_name_),
			unmake
		};
	...
	} else if (device_->getDriverKey() == "HackRF") {
		device_->setGainMode(SOAPY_SDR_RX, 0, false);

		double lna = options_.count("sdr-gain-lna") ? options_["sdr-gain-lna"].as<double>() : device_->getGainRange(SOAPY_SDR_RX, 0, "LNA").maximum();
		double vga = options_.count("sdr-gain-vga") ? options_["sdr-gain-vga"].as<double>() : device_->getGainRange(SOAPY_SDR_RX, 0, "VGA").maximum();
		double amp = options_.count("sdr-gain-amp") ? options_["sdr-gain-amp"].as<double>() : device_->getGainRange(SOAPY_SDR_RX, 0, "AMP").maximum();

		std::cerr << "SoapySDR: using HackRF per-element gain LNA=" << std::fixed << std::setprecision(1) << lna << " dB, VGA=" << vga << " dB, AMP=" << amp << " dB" << std::endl;

		device_->setGain(SOAPY_SDR_RX, 0, "LNA", lna);
		device_->setGain(SOAPY_SDR_RX, 0, "VGA", vga);
		device_->setGain(SOAPY_SDR_RX, 0, "AMP", amp);
	...
	try {
		stream_ = {
			device_->setupStream(SOAPY_SDR_RX, soapy_format, channels, stream_settings),
...
void SoapySampleSource::Run() {
	...
	while (!halt_) {
		...
		auto elements_read = device_->readStream(
				stream_.get(),
				buffs,
				elements,
				flags,
