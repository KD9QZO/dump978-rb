...
static int realmain(int argc, char *argv[]) {
	...
	desc.add_options()
			...
			("sdr-gain-vga", po::value<double>(), "Set VGA gain on HackRF devices")
			("sdr-gain-lna", po::value<double>(), "Set LNA gain on HackRF devices")
			("sdr-gain-amp", po::value<double>(), "Set AMP gain on HackRF devices")
	...
	if (sample_source) {
		...
		sample_source->SetConsumer(
				[receiver](std::uint64_t timestamp, const Bytes &buffer) {
	...
	message_source->SetErrorHandler(
			[&io_service, &saw_error](const boost::system::error_code &ec) {
	...
	signals.async_wait(
			[&io_service, &saw_error](const boost::system::error_code &ec, int signum) {
