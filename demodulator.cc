...
SingleThreadReceiver::SingleThreadReceiver(SampleFormat format): converter_(SampleConverter::Create(format)),
...
#ifdef AUTO_CENTER
// check that there is a valid sync word starting at 'phase' that matches the sync word 'pattern'.
// Return a pair:
// first element is true if the sync word looks OK; second element has the dphi threshold to use for bit slicing
static inline std::pair<bool, std::int16_t> CheckSyncWord(PhaseBuffer::const_iterator phase, std::uint64_t pattern) {
	...
	// find mean dphi for zero and one bits; take the mean of the two as our central value
...
boost::optional<Demodulator::Message> TwoMegDemodulator::DemodOneDownlink(PhaseBuffer::const_iterator start) {
#ifdef AUTO_CENTER
	auto sync = CheckSyncWord(start, DOWNLINK_SYNC_WORD); if (!sync.first) {
	...
	return Demodulator::Message{
		std::move(corrected),
		errors,
		start,
		start + (SYNC_BITS + bits) * 2
	};
}

boost::optional<Demodulator::Message> TwoMegDemodulator::DemodOneUplink(PhaseBuffer::const_iterator start) {
#ifdef AUTO_CENTER
	auto sync = CheckSyncWord(start, UPLINK_SYNC_WORD); if (!sync.first) {
		// Sync word had errors
		return boost::none;
